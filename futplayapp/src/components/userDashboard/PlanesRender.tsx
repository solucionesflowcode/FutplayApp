...
export default function PlanesRender() {
    ...
    useEffect(() => {
        const fetchMembresiaYPlanes = async () => {
            ...
            try {
                // 1. Verificar si el usuario tiene membresía
                ...
                // 2. Si no tiene plan, obtener los planes disponibles
                if (!membresias || membresias.length === 0) {
                    const { data: planesData, error: errorPlanes } = await supabase
                        .from("plan")
                        .select("*")
                        .order("precio", { ascending: true })
                        .limit(3); // Mostrar un par de planes promocionales

                    if (errorPlanes) {
                        console.error("Error obteniendo planes:", errorPlanes);
                    } else if (planesData) {
    ...
    if (loading) return null; // O un esqueleto si queremos que se vea
    if (hasPlan) return null; // Si tiene plan, no mostramos esta sección
