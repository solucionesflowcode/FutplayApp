...
export async function POST(request: Request) {
  ...
  try {
    ...
    const fileName = `capsula_${Date.now()}.${ext}`;
    const bytes = await file.arrayBuffer();

    const { error: bucketError } = await adminClient.storage.getBucket("capsulas");
    if (bucketError) {
      const { error: createError } = await adminClient.storage.createBucket("capsulas", {
        public: true,
    ...
    const { error: uploadError } = await adminClient.storage.from("capsulas").upload(fileName, bytes, {
    ...
    const { data: publicUrlData } = adminClient.storage.from("capsulas").getPublicUrl(fileName);
